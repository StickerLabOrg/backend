...
def test_register_user_ok(mocker, db_session):
    data = MagicMock(nome="Lucas", email="lucas@example.com", password="123", time_do_coracao="Palmeiras")
...
def test_register_user_email_in_use(mocker, db_session):
    ...
    mocker.patch("src.usuario.service.user_service.get_user_by_email",
...
def test_change_password_ok(mocker, db_session):
    fake_user = DummyUser(1, "Lucas", "lucas@example.com", "Palmeiras", password_hash="old_hash")
