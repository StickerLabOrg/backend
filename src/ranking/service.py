...
def _query_base(db: Session, filtro=None):
    acertos_expr = func.sum(
        case((Palpite.acertou == True, 1), else_=0)
    ).label("acertos")
    ...
    q = (
        db.query(
            User.id,
            User.nome,
            User.coins,  # agora coins entram no ranking
            total_expr,
            acertos_expr,
        )
